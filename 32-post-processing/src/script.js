...
gui.add(unrealBloomPass, 'strength').min(0).max(5).name('Strength')
gui.add(unrealBloomPass, 'radius').min(0).max(5).name('Radius')
gui.add(unrealBloomPass, 'threshold').min(0).max(5).name('Threshold')
...
const DisplacementShader = {
    ...
    fragmentShader: `
        ...
        void main()
        {
            ...
            vec2 newUv = vec2(
                // vUv.x ,
                vUv.x + sin(vUv.y * 5.0 + uTime *0.1) * 0.1,
                // vUv.y
                vUv.y + cos(vUv.x * 10.0 + uTime *0.1 ) * 0.1
            ) + normalColor.xy * 0.0;

...
{
    createCube(
        Math.random() * 0.8, Math.random() * 0.8, Math.random() * 0.9, 
        {
            x: (Math.random() - 0.5) * 3,
            y: 3,
            z: (Math.random() - 0.5) * 3
...
const createCube = (width, height, depth, position) =>
{
    ...
    // Cannon.js Body
    const cubeShape = new CANNON.Box(new CANNON.Vec3(width * 0.5, height * 0.5, depth *0.5))
...
createCube(1, 1,1, {x: 0, y: 1, z: -1.5})
